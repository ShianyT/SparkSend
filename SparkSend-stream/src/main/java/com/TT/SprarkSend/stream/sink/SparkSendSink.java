package com.TT.SprarkSend.stream.sink;

import cn.hutool.core.date.DateUtil;
import cn.hutool.core.text.CharSequenceUtil;
import cn.hutool.core.text.StrPool;
import com.TT.SparkSend.common.constant.SparkSendConstant;
import com.TT.SparkSend.common.domain.AnchorInfo;
import com.TT.SparkSend.common.domain.SimpleAnchorInfo;
import com.TT.SprarkSend.stream.utils.LettuceRedisUtils;
...
/**
 * @Description 消息进 redis/hive
 * @Author TT
 * @Date 2025/2/25
 */
@Slf4j
public class SparkSendSink implements SinkFunction<AnchorInfo> {
    @Override
    ...
    private void realTimeData(AnchorInfo info) {
        try {
            // 匿名方法
            // RedisPipelineCallBack.invoke()
            LettuceRedisUtils.pipeline(redisAsyncCommands -> {
                List<RedisFuture<?>> redisFutures = new ArrayList<>();

                /**
                 * 构建messageId维度的链路信息 数据结构 list:{key,list}
                 * key:SparkSend:MessageId:{messageId},listValue:[{timestamp,state,businessId},{timestamp,state,businessId}]
                 */
                String redisMessageKey = CharSequenceUtil.join(StrPool.COLON, SparkSendConstant.CACHE_KEY_PREFIX, SparkSendConstant.MESSAGE_ID, info.getMessageId());
                ...
                redisFutures.add(redisAsyncCommands.lpush(
                        redisMessageKey.getBytes(),
                        JSON.toJSONString(messageAnchorInfo).getBytes()
                ));
                redisFutures.add(redisAsyncCommands.expire(
                        redisMessageKey.getBytes(),
                        Duration.ofDays(3).toMillis() / 1000
                ));

                /**
                 * 构建userId维度的链路信息 数据结构 list:{key,list}
                ...
                for (String id : info.getIds()) {
                    redisFutures.add(redisAsyncCommands.lpush(
                            id.getBytes(),
                            JSON.toJSONString(userAnchorInfo).getBytes()
                    ));
                    redisFutures.add(redisAsyncCommands.expire(
                            id.getBytes(),
                ...
                /**
                 * 构建消息模板维度的链路信息 数据结构 hash:{key,hash}
            ...
            log.error("SparkSendSink#invoke error: {}", Throwables.getStackTraceAsString(e));
        }
    }
}
