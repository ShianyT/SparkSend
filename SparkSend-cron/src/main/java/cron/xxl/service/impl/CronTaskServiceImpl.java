...
public class CronTaskServiceImpl implements CronTaskService {
    ...
    public BasicResultVO saveCronTask(XxlJobInfo xxlJobInfo) {
        ...
        try {
            response = HttpRequest.post(path).form(params).cookie(getCookie()).execute();// 发送给xxl创建定时任务
    ...
    private String getCookie() {
        String cashedCookie = redisTemplate.opsForValue().get(XxlJobConstant.COOKIE_PREFIX + xxlUserName);
        if (StrUtil.isNotBlank(cashedCookie)) {
            return cashedCookie;
        }
